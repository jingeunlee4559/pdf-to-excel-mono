  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 lg:grid lg:grid-cols-2">
      <section className="hidden min-h-screen overflow-hidden bg-slate-950 lg:block">
        <div className="flex h-full flex-col justify-between bg-gradient-to-br from-brand-900 via-brand-700 to-emerald-600 p-12 text-white">
          <div>
            ...
            <p className="mt-5 max-w-lg text-base leading-8 text-white/75">OCR 없이 텍스트 기반 문서 파싱, 표 후보 생성, 사용자 수정, 자사 양식 엑셀 다운로드 흐름만 남긴 버전입니다.</p>
          </div>
      ...
      <main className="flex min-h-screen items-center justify-center px-5 py-10 sm:px-10">
        <div className="w-full max-w-[430px]">
          <div className="text-center">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-3xl bg-brand-600 text-xl font-black text-white shadow-glow lg:hidden">AI</div>
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            ...
            <button disabled={loading} className="h-[52px] w-full rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white transition hover:bg-brand-700 disabled:bg-slate-400">{loading ? '로그인 중...' : '로그인'}</button>
          </form>
