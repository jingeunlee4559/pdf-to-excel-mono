...
function formatFileProfiles(fileProfiles = []) {
  if (!Array.isArray(fileProfiles) || !fileProfiles.length) return '';
  return fileProfiles.slice(0, 8).map((file) => {
    const name = file.fileName || `파일${file.index || ''}`;
    const type = file.documentType || '업무 문서';
    const role = file.roleLabel || file.role || '분석 대상';
    const summary = file.summary || `${file.pageCount || 0}페이지 / ${file.rowCount || 0}행`;
    return `- ${name}: ${type} / ${role} / ${summary}`;
  }).join('\n');
}
...
function fallbackChat(message, context = {}, errorMessage = '') {
  ...
  if (intent === 'DOCUMENT_QA') {
    if (hasDocument) {
      return {
        ...base,
        answer: (() => {
          const profileText = formatFileProfiles(analysis.fileProfiles || analysis.raw?.fileProfiles || []);
          if (profileText && /(각각|파일별|문서별|유형|어떤\s*파일|무슨\s*파일)/i.test(text)) {
            return `현재 분석 결과는 ${analysis.documentType || '업무 문서'}입니다. 파일별 유형은 다음과 같습니다.\n${profileText}`;
          }
          return `현재 문서는 ${analysis.documentType || '업무 문서'}로 보입니다. ${analysis.summary || ''} 확인 필요 항목은 ${issues.length || 0}건입니다.`;
        })(),
        intent: 'DOCUMENT_QA',
        needsFile: false,
  ...
  if (intent === 'PRICE_COMPARE') {
    ...
    return {
      ...base,
      answer: `현재 저장된 표 데이터 ${rows.length}행 기준으로 단가 비교가 가능합니다. 파일을 다시 올리지 않고 같은 채팅의 분석 결과를 사용합니다.`,
      intent: 'PRICE_COMPARE',
      needsFile: false,
      action: 'SHOW_TABLE',
